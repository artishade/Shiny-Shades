            ...
            <div className="relative z-10 px-8 md:px-16 py-8 md:py-10 flex flex-col md:flex-row items-center gap-8">
                <div className="flex-1 text-center md:text-left">
                    ...
                    <p className="text-sm md:text-base mb-0 max-w-xs md:max-w-sm text-white/80" style={{ lineHeight: '1.7' }}>
                        {banner?.subtitle || 'Exclusive savings on timeless elegance and premium lingerie.'}
                    </p>
                </div>
            </div>
        </motion.div>
...
export const SalePage: React.FC = () => {
    ...
    // Base list of sale products
    const baseSaleProducts = useMemo(() => {
        return allProducts.filter(
            p => p.isOnSale === true || (p.comparePrice != null && Number(p.comparePrice) > Number(p.price))
        );
    }, [allProducts]);

    // Dynamic filter options extracted from the current sale items
    const availableFilters = useMemo(() => {
        return extractAvailableFilters(baseSaleProducts);
    }, [baseSaleProducts]);
                        ...
                        <div className="flex-1 min-w-0">
                            {loading ? (
                                <div className="text-center py-20">
                                    ...
                                    <p className="text-[#6B5B55]">Loading sale items...</p>
                                </div>
